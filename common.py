...
def precondition(pre_condition, error_message):
    ...
    if not pre_condition:
        logger.error(
            f"Pre-condition: {pre_condition}. Error message: {error_message}")
...
def dns_lookup_with_retry(domain_name, record_type, total_retry_count, dns_servers=[]):
    ...
    while attempt <= total_retry_count:
        lookup_result_per_attempt = dns_lookup(
            domain_name, record_type, dns_servers)
        dns_lookup_result_set = set(
            lookup_result_per_attempt) | dns_lookup_result_set
        logger.info(
...
def get_elb_authoritative_name_server_ip_list(elb_dns_name):
    ...
    # ELB name cannot contain dot. It must be at most 32 characters in length, must contain only alphanumeric characters or hyphens,
    ...
    logger.info(
        f"Authoritative name server domain set: {authoritative_server_dns_set}")
    for authoritative_server_dns_name in authoritative_server_dns_set:
        authoritative_server_ip_list += dns_lookup(
            authoritative_server_dns_name, "A")
    logger.info(
        f"Authoritative name server IP list: {authoritative_server_ip_list}")
...
def get_pending_registration_ip_set(
    ip_from_dns_set, ip_from_target_group_set, active_ip_from_previous_invocation_set
):
    """
    ...
    # 3. Those IPs are not in the active IP list from the previous invocation
    :param active_ip_from_previous_invocation_set: a set of active IPs from the previous invocation
    ...
    ip_in_dns_not_in_target_group = ip_from_dns_set - ip_from_target_group_set
    if not active_ip_from_previous_invocation_set:
        logger.info("There are no active IPs from the previous invocation")
        return ip_in_dns_not_in_target_group

    # When there are active IPs from the previous invocation. Remove them.
    logger.info(
        f"Active IP from previous invocation - {active_ip_from_previous_invocation_set}"
    )
    pending_registration_ip_set = (
        ip_in_dns_not_in_target_group - active_ip_from_previous_invocation_set
    )
...
def get_invocation_count_per_pending_deregistration_ip(
    ...
    logger.info(
        "No pending deregistration IP found from the previous invocations")
